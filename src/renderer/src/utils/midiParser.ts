...
const VENUE_TRACK_NAME = 'VENUE'

// RB3/RBN2 spotlight notes (note-on duration = event duration)
const VENUE_SPOTLIGHT_NOTES: Record<number, VenuePerformerEvent['performer']> = {
  37: 'bass',
  38: 'drums',
  39: 'guitar',
  40: 'vocals',
...
// RB1/RB2/RBN1 sing-along notes
const VENUE_SINGALONG_NOTES: Record<number, VenuePerformerEvent['performer']> = {
  85: 'bass',
  86: 'drums',
  87: 'guitar'
}
// RB1/RB2/RBN1 lighting keyframe notes → stage effects [next]/[prev]/[first]
const VENUE_KEYFRAME_NOTES: Record<number, string> = {
  48: 'next',
  49: 'prev',
  50: 'first'
}
...
function parseVenueTextEvent(venueTrack: VenueTrackData, tick: number, eventText: string): void {
  ...
  if (performerMatch) {
    venueTrack.performer.push({
      id: uuidv4(),
      tick,
      type: performerMatch[1].toLowerCase(),
...
function parseVenueNote(venueTrack: VenueTrackData, noteNumber: number, tick: number, duration: number): void {
  ...
  if (noteNumber in VENUE_KEYFRAME_NOTES) {
    venueTrack.stage.push({ id: uuidv4(), tick, effect: VENUE_KEYFRAME_NOTES[noteNumber] })
    return
  }
  if (noteNumber in VENUE_CAMERA_NOTES) {
    venueTrack.cameraCuts.push({ id: uuidv4(), tick, subject: VENUE_CAMERA_NOTES[noteNumber] })
    return
  }
...
export function parseMidiBase64(midiBase64: string): ParsedMidiData {
  ...
  const laneMarkers: LaneMarker[] = []
  const songSections: SongSection[] = []
  const venueTrack = createEmptyVenueTrack()
  ...
  // Parse marker/text events for global song sections and VENUE track events from raw MIDI tracks.
  for (const rawTrack of rawMidi.tracks) {
    let rawTrackName = ''
    for (const ev of rawTrack) {
      if (ev.type === 'trackName' && 'text' in ev) {
    ...
    let absTick = 0
    const venueNoteStarts = new Map<number, number>()
    for (const ev of rawTrack) {
      absTick += ev.deltaTime || 0
      const tick = Math.round(absTick * tickScale)

      if ((ev.type === 'marker' || ev.type === 'text') && 'text' in ev) {
        const rawText = String((ev as RawMidiEvent & { text: string }).text || '').trim()
        if (!rawText) continue

        const bracketMatch = rawText.match(/^\[section\s+(.+)]$/i)
        const plainMatch = rawText.match(/^section\s+(.+)$/i)
        const name = (bracketMatch?.[1] || plainMatch?.[1] || '').trim()
        if (name) {
          songSections.push({ id: uuidv4(), tick, name })
          continue
        }
      ...
      if (rawTrackName !== VENUE_TRACK_NAME || !('noteNumber' in ev)) continue

      if (ev.type === 'noteOn') {
        const noteEvent = ev as RawMidiEvent & { noteNumber: number; velocity: number }
        if (noteEvent.velocity > 0) {
          venueNoteStarts.set(noteEvent.noteNumber, tick)
        } else {
          const startTick = venueNoteStarts.get(noteEvent.noteNumber)
          if (startTick !== undefined) {
            parseVenueNote(venueTrack, noteEvent.noteNumber, startTick, tick - startTick)
            venueNoteStarts.delete(noteEvent.noteNumber)
          }
        }
      } else if (ev.type === 'noteOff') {
        const noteEvent = ev as RawMidiEvent & { noteNumber: number }
        const startTick = venueNoteStarts.get(noteEvent.noteNumber)
        if (startTick !== undefined) {
          parseVenueNote(venueTrack, noteEvent.noteNumber, startTick, tick - startTick)
          venueNoteStarts.delete(noteEvent.noteNumber)
  ...
  songSections.sort((a, b) => a.tick - b.tick)
  venueTrack.lighting.sort((a, b) => a.tick - b.tick)
  venueTrack.postProcessing.sort((a, b) => a.tick - b.tick)
  ...
  laneMarkers.sort((a, b) => a.tick - b.tick)
  ...
  if (eventsSection) {
    ...
    for (const line of eventsSection.lines) {
      ...
      if (eventText === 'phrase_start') {
        ...
        const bracketMatch = eventText.match(/^\[section\s+(.+)]$/i)
        const plainMatch = eventText.match(/^section\s+(.+)$/i)
        const name = (bracketMatch?.[1] || plainMatch?.[1] || '').trim()
        if (name) {
          songSections.push({ id: uuidv4(), tick, name })
        } else {
          parseVenueTextEvent(venueTrack, tick, eventText)
        }
      }
    }
...
export function serializeChartFile(
  ...
  for (const sec of songSections) {
    const tick = scaleTick(sec.tick)
    lines.push(`  ${tick} = E "section ${sec.name}"`)
  }
  for (const event of buildVenueTextEvents(venueTrack)) {
    const tick = scaleTick(event.tick)
    lines.push(`  ${tick} = E "${event.text}"`)
  }
  lines.push('}')
