...
export function VenueStateOverlay({ songId }: { songId: string }): React.JSX.Element {
    ...
    const draw = (): void => {
      ...
      for (const note of filteredNotes) {
        ...
        if (isTalkie) {
          ...
          // Label
          if (nw > 40) {
            ctx.globalAlpha = isPast ? 0.3 : isActivePart ? 0.8 : 0.3
            ctx.fillStyle = '#fff'
            ctx.font = 'bold 10px sans-serif'
            ctx.textAlign = 'center'
            ctx.textBaseline = 'middle'
            ctx.fillText('TALKIE', nx + nw / 2, ch / 2)
          }
          if (note.lyric && isActivePart && editingLyric?.noteId !== note.id) {
            ctx.globalAlpha = isPast ? 0.35 : 1
            ctx.fillStyle = isCurrent ? '#14b8a6' : '#fff'
            ctx.font = '600 13px sans-serif'
            ctx.textAlign = 'center'
            ctx.textBaseline = 'middle'
            ctx.shadowColor = 'rgba(0,0,0,0.9)'
            ctx.shadowBlur = 3
            ctx.fillText(note.lyric, nx + nw / 2, ch / 2, Math.max(nw - 10, 30))
          }
          ctx.restore()
          rects.push({ id: note.id, x: nx, y: 0, w: nw, h: ch })
          continue
        }
      ...
      {open && panelStyle && (
        ...
        >
          ...
          <div className="shortcut-help-section">
            ...
            <ShortcutRow keys={hotkeys.toolSelect} desc="Select tool" />
            <ShortcutRow keys={hotkeys.toolPlace} desc="Place tool" />
            <ShortcutRow keys={hotkeys.toolErase} desc="Erase tool" />
          </div>
          <div className="shortcut-help-section">
          ...
          <div className="shortcut-help-section">
            ...
            <ShortcutRow keys={hotkeys.copy} desc="Copy selected notes" />
            <ShortcutRow keys={hotkeys.paste} desc="Paste at playhead" />
            <ShortcutRow keys={hotkeys.undo} desc="Undo" />
            <ShortcutRow keys={hotkeys.redo} desc="Redo" />
            <ShortcutRow keys={hotkeys.selectAll} desc="Select all" />
            <ShortcutRow keys={hotkeys.deleteSelection} desc="Delete selected" />
            <ShortcutRow keys={hotkeys.createStarPower} desc="Star Power from selection" />
            <ShortcutRow keys={hotkeys.createSolo} desc="Solo from selection" />
            <ShortcutRow keys="Esc" desc="Clear selection" />
          </div>
          <div className="shortcut-help-section">
            <div className="shortcut-help-section-title">Playback &amp; View</div>
            <ShortcutRow keys={hotkeys.playPause} desc="Play / Pause" />
            <ShortcutRow keys={hotkeys.zoomIn} desc="Zoom in" />
            <ShortcutRow keys={hotkeys.zoomOut} desc="Zoom out" />
            <ShortcutRow keys={hotkeys.resetZoom} desc="Reset zoom" />
            <ShortcutRow keys={hotkeys.save} desc="Save" />
          </div>
          <div className="shortcut-help-section">
