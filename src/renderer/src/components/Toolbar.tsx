...
export function Toolbar(): React.JSX.Element {
  const { activeSongId, loadFolder } = useProjectStore()
  ...
  useEffect(() => {
    return window.api.onAutoChartComplete((event) => {
      ...
      const newSongId = event.songFolders && event.songFolders.length > 0
      ...
      if (event.outputDir) {
        ...
        if (event.success && autoChartDownloadVideo && event.urlSongFolders && event.urlSongFolders.length > 0) {
          void Promise.allSettled(
            ...
            void loadFolder(event.outputDir, newSongId)
          })
        } else {
          void loadFolder(event.outputDir, newSongId)
        }
  ...
  }, [autoChartDownloadVideo, loadFolder, updateSettings])
