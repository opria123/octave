...
export function Toolbar(): React.JSX.Element {
  ...
  // Reactively subscribe to song store state so UI updates when isPlaying changes
    ...
    <div className="toolbar">
      ...
      <div className="toolbar-group" style={{ position: 'relative' }}>
        <button
          ...
          onClick={() => setShowOpenDropdown((prev) => !prev)}
          title="Open Folder or Import Package"
        >
      ...
      <div className="toolbar-group toolbar-volume">
        ...
        />
        <StemMixerButton key={activeSongId ?? 'none'} activeSongId={activeSongId} />
      </div>
                ...
                {validationIssues.map((issue, i) => (
                  ...
                  >
                    <span
                      style={{
                        fontWeight: 600,
                        color: issue.severity === 'error' ? '#f88' : '#ffcc44'
                      }}
                    >
...
function StemMixerButton({
  ...
  const [stems, setStems] = useState<audioService.StemControl[]>(() =>
    activeSongId ? audioService.getStemControls(activeSongId) : []
  )
